...
export default function App() {
      ...
      .on(
        ...
        (payload) => {
          if (payload.eventType === "UPDATE" && payload.new.status === "active" && payload.old.status === "waiting") {
            toast({
              title: "🏁 Game Started!",
              description: "The typing race has begun! Join the next game.",
              duration: 5000,
            });
            if (appState === "landing" || appState === "entry") {
              setCurrentGame(null);
            }
          }
          fetchActiveGame();
  ...
  const fetchActiveGame = async () => {
    if (appState === "landing" || appState === "entry") {
      ...
      setCurrentGame(data as Game | null);
  ...
  const handleJoinGame = () => {
    ...
    if (currentGame.status === "active") {
      toast({
        ...
        description: "Please wait for the next game to be hosted.",
        variant: "destructive",
      });
      return;
    }
  ...
  const handleEntry = async (nickname: string, wallet: string) => {
    ...
    // Check if game is still in waiting status before allowing join
    ...
    if (gameCheck?.status !== "waiting") {
      toast({
        title: "Game Already Started",
        description: "This game has already begun. Please wait for the next game.",
        variant: "destructive",
      });
      setAppState("landing");
      setCurrentGame(null);
